...
namespace Memoria.Prime.NVorbis
{
	abstract class VorbisResidue
	{
		internal static VorbisResidue Init(VorbisStreamDecoder vorbis, DataPacket packet)
		{
			...
			if (residue == null) throw new InvalidOperationException();
		...
		class Residue0 : VorbisResidue
		{
			...
			protected override void Init(DataPacket packet)
			{
				...
				for (var i = 0; i < acc; i++)
				{
					...
					if (_vorbis.Books[bookNums[i]].MapType == 0) throw new InvalidOperationException();
				}
				...
				while (dim > 0)
				{
					...
					if (partvals > entries) throw new InvalidOperationException();
					--dim;
				}
