...
namespace Memoria.Prime.NVorbis
{
	class VorbisStreamDecoder : IVorbisStreamStatus, IDisposable
	{
		...
		internal bool TryInit()
		{
			...
			{
				throw new InvalidOperationException("Comment header was not readable!");
			}
			packet.Done();
			...
			if (!LoadBooks(packet))
			{
				throw new InvalidOperationException("Book header was not readable!");
			}
			packet.Done();
		...
		void ProcessParameterChange(DataPacket packet)
		{
			...
			if (ProcessStreamHeader(packet))
			{
				...
				if (packet == null) throw new InvalidOperationException("Couldn't get next packet!");
			}
		...
		bool ProcessStreamHeader(DataPacket packet)
		{
			...
			if (packet.ReadInt32() != 0) throw new InvalidOperationException("Only Vorbis stream version 0 is supported.");
		...
		bool LoadBooks(DataPacket packet)
		{
			...
			if (!packet.ReadBit()) throw new InvalidOperationException();
