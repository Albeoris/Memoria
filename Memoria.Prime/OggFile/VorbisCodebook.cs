...
namespace Memoria.Prime.NVorbis
{
	class VorbisCodebook
	{
		...
		internal void Init(DataPacket packet)
		{
			...
			if (chkVal != 0x564342UL) throw new InvalidOperationException();
		...
		void InitTree(DataPacket packet)
		{
			...
			if ((MaxBits = Lengths.Max()) > -1)
			{
				...
				if (!ComputeCodewords(sparse, sortedEntries, codewords, codewordLengths, len: Lengths, n: Entries, values: values)) throw new InvalidOperationException();
