...
namespace Memoria.Prime.NVorbis
{
	abstract class VorbisFloor
	{
		internal static VorbisFloor Init(VorbisStreamDecoder vorbis, DataPacket packet)
		{
			...
			if (floor == null) throw new InvalidOperationException();
		...
		class Floor0 : VorbisFloor
		{
			...
			protected override void Init(DataPacket packet)
			{
				...
				if (_order < 1 || _rate < 1 || _bark_map_size < 1 || _books.Length == 0) throw new InvalidOperationException();
				...
				for (int i = 0; i < _books.Length; i++)
				{
					...
					if (num < 0 || num >= _vorbis.Books.Length) throw new InvalidOperationException();
					var book = _vorbis.Books[num];

					if (book.MapType == 0 || book.Dimensions < 1) throw new InvalidOperationException();
		...
		class Floor1 : VorbisFloor
		{
			...
			protected override void Init(DataPacket packet)
			{
				...
				for (int i = 0; i < _sortIdx.Length - 1; i++)
				{
					...
					{
						if (_xList[i] == _xList[j]) throw new InvalidOperationException();
