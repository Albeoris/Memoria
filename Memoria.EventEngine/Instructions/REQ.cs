...
namespace FF8.JSM.Instructions
{
    ...
    internal sealed class REQ : JsmInstruction
    {
        ...
        private REQ(IJsmExpression scriptLeve, IJsmExpression entry, IJsmExpression function)
        {
        ...
        {
            return $"{nameof(REQ)}({nameof(_scriptLeve)}: {_scriptLeve}, {nameof(_entry)}: {_entry}, {nameof(_function)}: {_function})";
        }
    }
}