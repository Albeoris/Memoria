...
public partial class EventEngine
{
    ...
    private void ReassignBasicAnimationForField(Actor actor)
    {
        if ((Int32)actor.idle != 0)
            AnimationFactory.AddAnimWithAnimatioName(actor.go, FF9DBAll.AnimationDB[(Int32)actor.idle]);
        if ((Int32)actor.walk != 0)
            AnimationFactory.AddAnimWithAnimatioName(actor.go, FF9DBAll.AnimationDB[(Int32)actor.walk]);
        if ((Int32)actor.run != 0)
            AnimationFactory.AddAnimWithAnimatioName(actor.go, FF9DBAll.AnimationDB[(Int32)actor.run]);
        if ((Int32)actor.turnl != 0)
            AnimationFactory.AddAnimWithAnimatioName(actor.go, FF9DBAll.AnimationDB[(Int32)actor.turnl]);
        if ((Int32)actor.turnr == 0)
            return;
        AnimationFactory.AddAnimWithAnimatioName(actor.go, FF9DBAll.AnimationDB[(Int32)actor.turnr]);
    }
}