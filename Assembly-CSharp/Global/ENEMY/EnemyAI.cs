		...
		for (Int32 i = 0; i < hand.Count; i++)
		{
			for (Int32 x = 0; x < Board.SIZE_X; x++)
			{
				for (Int32 y = 0; y < Board.SIZE_Y; y++)
				{
					...
					{
						Int32 flipCount = Check(board, hand[i], new Vector2((Single)x, (Single)y), out defender);
						if (flipCount > 0)
						{
						...
						{
							BattleCalculation battleCalculation = null;
                            if (Configuration.Mod.TranceSeek || (Configuration.TetraMaster.TripleTriad > 0))
							{
                                battleCalculation = QuadMistGame.main.CalculateTripleTriad(hand[i], defender, x, y);
                            }
							else
							{
                                battleCalculation = QuadMistGame.main.Calculate(hand[i], defender);
							}
                            Vector2 cardLocation = board.GetCardLocation(hand[i]);
        ...
        for (Int32 i = 0; i < adjacentCards.Length; i++)
        {
            if (adjacentCards[i] != null)
            {
                ...
                if (quadMistCard.side != yourCard.side)
                {
					...
					{
                        TripleTriadCard baseCardAttacker = TripleTriad.TripleTriadCardStats[(TripleTriadId)yourCard.id];
                        TripleTriadCard baseCardDefender = TripleTriad.TripleTriadCardStats[(TripleTriadId)quadMistCard.id];
