...
public class EndGame
{
	public EndGame()
	{
		this.ff9endingGameBankroll = 1000L;
		this.count = -1;
		this.wx = new Int64[3];
		this.wy = new Int64[3];
	...
	public void OnUpdate()
	{
		...
		case EndGameDef.FF9EndingGameState.ENDGAME_STATE_WAGER:
			...
			if (EndSys.Endsys_GetMode() == EndSys.ENDSYS_MODE_ENUM.ENDSYS_MODE_QUIT)
			{
				...
				this.DBG_PRINT("ff9endingGameBankroll: " + this.ff9endingGameBankroll);
			}
			break;
		...
		case EndGameDef.FF9EndingGameState.ENDGAME_STATE_PROCESSPLAYER:
			...
			if ((this.optionFlags & 2UL) != 0UL && this.ff9endingGameWager * 2L <= this.ff9endingGameBankroll)
			{
				this.ff9endingGameDoubleAllowed = true;
			}
			if ((this.optionFlags & 1UL) != 0UL && this.isSplit == 0 && this.ff9endingGameWager * 2L <= this.ff9endingGameBankroll)
			{
			...
			if (this.count == -1)
			{
				if (this.isSplit == 1)
				{
					if (this.player.cardTotal > 21u)
					{
						...
						this.ff9endingGameBankroll -= this.ff9endingGameWager * (Int64)(((this.player.flags & 4u) == 0u) ? 1L : 2L);
					}
				...
				else if (this.split.cardTotal > 21u)
				{
					this.resultType = EndGameDef.ENDGAME_RESULT_ENUM.ENDGAME_RESULT_LOSE;
					this.FF9SFX_Play(107u);
					this.ff9endingGameBankroll -= this.ff9endingGameWager * (Int64)(((this.split.flags & 4u) == 0u) ? 1L : 2L);
				}
		...
		case EndGameDef.FF9EndingGameState.ENDGAME_STATE_BLACKJACK:
			if (this.count == -1)
			{
				...
				{
					this.FF9SFX_Play(108u);
					this.ff9endingGameBankroll += this.ff9endingGameWager + (this.ff9endingGameWager >> 1);
				}
				else
				{
					this.FF9SFX_Play(107u);
					this.ff9endingGameBankroll -= this.ff9endingGameWager;
				}
				this.count = 148;
				this.dealer.flags &= 4294967279u;
			}
		...
		case EndGameDef.FF9EndingGameState.ENDGAME_STATE_WIN:
			...
			else
			{
				...
				this.DBG_PRINT("bankroll: " + this.ff9endingGameBankroll);
			}
			break;
		...
		case EndGameDef.FF9EndingGameState.ENDGAME_STATE_NEXTHAND:
			...
			{
				this.isSplit = 0;
				if (this.ff9endingGameWager > this.ff9endingGameBankroll)
				{
					this.ff9endingGameWager = this.ff9endingGameBankroll / 10L * 10L;
				}
				this.count = -1;
				this.ff9endingGameState = EndGameDef.FF9EndingGameState.ENDGAME_STATE_WAGER;
				EndSys.Endsys_SetMode(EndSys.ENDSYS_MODE_ENUM.ENDSYS_MODE_BET);
		...
		case EndGameDef.FF9EndingGameState.ENDGAME_STATE_LEAVEGAME:
			if (this.count == -1)
			{
				...
				if (this.ff9endingGameBankroll < 10L)
				{
					this.ff9endingGameBankroll = 1000L;
				}
			}
		...
		if (this.ff9endingGameState < EndGameDef.FF9EndingGameState.ENDGAME_STATE_END && this.ff9endingGameBankroll < 10L)
		{
			this.count = -1;
			this.ff9endingGameState = EndGameDef.FF9EndingGameState.ENDGAME_STATE_LEAVEGAME;
		}
	...
	public Int64 ff9endingGameBankroll;
