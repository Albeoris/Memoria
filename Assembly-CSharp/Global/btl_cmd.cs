    ...
    */
    ...
    public static Boolean CheckMagicCondition(CMD_DATA cmd)
    {
        if (!btl_stat.CheckStatus(cmd.regist, BattleStatus.Silence) || (cmd.AbilityCategory & 2) == 0)
            return true;
        if (Configuration.Mod.TranceSeek && btl_stat.CheckStatus(cmd.regist, BattleStatus.EasyKill)) // Trance Seek - Bosses can use magic under silence (but have malus in Memoria.Scripts).
            return true;
