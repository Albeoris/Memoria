...
public abstract class SFXDataMesh
{
    ...
    public class EffectMaterial
    {
        ...
        public FilterMode filterMode = ModelFactory.GetFilterMode(Configuration.Graphics.SFXSmoothTexture, 0);
            ...
            if (textureKind == TextureKind.BACKGROUND)
                for (Int32 i = backgroundCapture.Count - 1; i >= 0; i--)
                    if (frame >= backgroundCapture[i].frame && backgroundCaptureLoaded < backgroundCapture[i].frame)
                    {
                        ...
                        renderTexture.filterMode = ModelFactory.GetFilterMode(Configuration.Graphics.SFXSmoothTexture, 1);
        ...
        public void ConvertFromSFXMesh(SFXMesh sfxmesh)
        {
            ...
            if (SFXKey.IsTexture(meshKey))
            {
                ...
                if (filter == SFXKey.FILTER_POINT)
                    ...
                    filterMode = ModelFactory.GetFilterMode(Configuration.Graphics.SFXSmoothTexture, 1);
                else
                    filterMode = ModelFactory.GetFilterMode(Configuration.Graphics.SFXSmoothTexture, 0);
    ...
    public class Raw : SFXDataMesh
    {
        ...
        public override void Begin()
        {
            if (genTextureMesh.Count > 0)
            {
                ...
                genTexture.filterMode = ModelFactory.GetFilterMode(Configuration.Graphics.SFXSmoothTexture, 1);
                genTexture.Create();
            }
            Raw.RenderingCount++;
        }
    ...
    public class JSON : SFXDataMesh
    {
        ...
        public override void Begin()
        {
            foreach (ModelSequence mseq in model)
            {
                ...
                {
                    tok.unityObject = ModelFactory.CreateModel(tok.fbxPath, false, true, Configuration.Graphics.SFXSmoothTexture);
